...
export default function ParticipantFormPage() {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    await axios.post(`${API_URL}/participants`, { name, email, phone });
    router.push('/');
  };
  return (
    <div className="min-h-screen bg-[#f9f9f6] flex flex-col items-center justify-center p-8">
      ...
      <form className="bg-white rounded-lg shadow p-8 border-2 border-[#1D1D1B] max-w-md w-full mx-auto" onSubmit={handleSubmit}>
        <h1 className="text-2xl font-bold mb-4 text-[#1D1D1B] text-center">Novo Participante</h1>
        ...
        <input className="border-2 border-[#1D1D1B] text-[#1D1D1B] rounded p-2 mb-2 w-full placeholder-[#1D1D1B]" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required />
        <input className="border-2 border-[#1D1D1B] text-[#1D1D1B] rounded p-2 mb-2 w-full placeholder-[#1D1D1B]" placeholder="Telefone" value={phone} onChange={e => setPhone(e.target.value)} required />
        <button className="bg-[#EEE416] text-[#1D1D1B] px-4 py-2 rounded mt-2 w-full font-semibold shadow hover:bg-[#e6d900] transition" type="submit">Criar Participante</button>
      </form>
    </div>
  );
}
